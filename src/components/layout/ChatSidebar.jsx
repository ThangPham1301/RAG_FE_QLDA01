    ...
    <aside className="flex w-full max-w-64 flex-col bg-[#eff4ff] py-6">
      ...
      <nav className="mt-6 space-y-1 px-3">
        {NAV_ITEMS.map(({ label, icon: Icon }) => {
          const active = label === 'Chat'
          const highlighted = label === 'Dashboard'
          ...
          return (
            <button
              ...
              } ${highlighted ? 'border-2 border-slate-900' : 'border-2 border-transparent'}`}
              aria-current={active ? 'page' : undefined}
              title={label}
            >
              <span
                className={`grid h-8 w-8 place-items-center rounded-md ${
                  active ? 'bg-blue-800 text-white' : 'bg-slate-500 text-slate-100'
                }`}
                aria-hidden="true"
              >
