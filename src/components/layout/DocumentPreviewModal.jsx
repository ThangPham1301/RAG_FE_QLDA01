...
const FILE_ICON = { pdf: '📄', docx: '📝', doc: '📝', txt: '📃', other: '📎' }
...
/* ─── Text viewer (fetches full extracted_text via /text API) ─── */
...
export default function DocumentPreviewModal({ document: doc, onClose }) {
    ...
    const tabs = [
        ...
        { id: 'text', label: `${FILE_ICON[doc.file_type] || '📋'} Nội dung văn bản` },
    ].filter(Boolean)

    const downloadUrl = hasUrl
        ? DocumentsAPI.previewUrl(doc.id) + '&download=1'
        : null
            ...
            >
                ...
                <div className="flex items-start justify-between gap-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white px-6 py-4">
                    <div className="flex min-w-0 items-start gap-3">
                        <span className="mt-0.5 shrink-0 text-2xl">{FILE_ICON[doc.file_type] || '📎'}</span>
